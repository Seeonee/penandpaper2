...
CharacterSupport.details = function(character) {
  ...
  if (heritage) {
    ...
    if (heritage) {
      var name = CaseUtils.capitalizeWords(heritage.name);
      details += ' ' + name;
    }
  }
  ...
  if (calling) {
    ...
    if (calling) {
      var name = CaseUtils.capitalizeWords(calling.name);
      details += ' ' + name;
    }
  }
  if (details.length == 0) {
...
var slot_ordering = {
  ...
  calling: 1,
  one_handed_weapon: 2,
  one_handed_weapon_upgrade: 2,
  two_handed_weapon: 4,
  two_handed_weapon_upgrade: 4,
  two_handed_ranged_weapon: 6,
  two_handed_ranged_weapon_upgrade: 6,
  one_handed_shield: 8,
  one_handed_shield_upgrade: 8,
  ability: 10,
  ability_upgrade: 10,
  armor: 12,
  armor_upgrade: 12,
...
// Get a character's equipped skills.
CharacterSupport.equipped_skills = function(character) {
  var skill_ids = [];
  var skill_slots = {};
  _.each(character.slots, function(value, key) {
    if ('equipped' in value) {
      var skill_id = value.equipped;
      skill_ids.push(skill_id);
      skill_slots[skill_id] = 0;
    } else {
      _.each(value, function(v, k) {
        var skill_id = v.equipped
        skill_ids.push(skill_id);
        skill_slots[skill_id] = k;
      });
    }
  });
  var skills = [];
  _.each(skill_ids, function(skill_id) {
    var skill = Codex.findOne({_id: skill_id});
    if (skill) {
      skills.push(skill);
    }
  });
  var temp = _.sortBy(skills, function(skill) {
    var upgrade = 0;
    if (skill.slots[0].indexOf('upgrade') >= 0) {
      upgrade = 10;
    }
    return (slot_ordering[skill.slots[0]] * 10000) + ((skill_slots[skill._id] + upgrade) * 100) + skill.level;
  });
  return temp;
}
...
CharacterSupport.get_owners = function(db) {
  ...
  db.find({}, {sort: {owner: 1, level: -1, name: 1}}).forEach(function(character) {
    ...
    owners[owner].characters.push({
      name: character.name,
      name_pretty: CaseUtils.capitalizeWords(character.name),
      details: CharacterSupport.details(character)
    });
  });
